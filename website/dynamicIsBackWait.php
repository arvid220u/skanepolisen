...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if ($_POST['specifier'] == "isBack") {
		mysqli_query_trace($con, "UPDATE ActiveUsers SET DynamicIsBack='1' WHERE Username='".$username."'");
		unset($_SESSION['setBusted']);
	}
}

checkDynamicIsBack("after", $username);

$usernames = getUsernamesList($con, "ActiveUsers");
$allAreBack = TRUE;
foreach ($usernames as $name) {
	if (returnFieldForUser("ActiveUsers", 13, $name) == 0) {
		$allAreBack = FALSE;
	}
}

if ($allAreBack) {
	// Reset all dynamicMode-specific properties, redirect to game.php
	
	// Start with ActiveGame
	mysqli_query_trace($con, "UPDATE ActiveGame SET DynamicMode='0'");
	mysqli_query_trace($con, "UPDATE ActiveGame SET DynamicPause='0'");
	mysqli_query_trace($con, "UPDATE ActiveGame SET DynamicModeStart=NULL");
	mysqli_query_trace($con, "UPDATE ActiveGame SET DynamicEnd='0'");
	mysqli_query_trace($con, "UPDATE ActiveGame SET GottmosLabelDisappear=NULL");
	// Then ActiveUsers for all users (then no need for foreach, as same action applies to all)
	...
	mysqli_query_trace($con, "UPDATE ActiveUsers SET InTransit='0'");
	mysqli_query_trace($con, "UPDATE ActiveUsers SET DynamicIsBack='0'");
	mysqli_query_trace($con, "UPDATE ActiveUsers SET DynamicEndPassed='0'");
...
<html lang="sv">
	...
	<body>
		...
		<div class="indented">
			...
			<header>
				<h1>Väntar på att alla spelare ska komma tillbaka...</h1>
			</header>
			
			<p>Än så länge har <?php 
				$count = 0;
				$plural = "";
				$isBacks = returnColumn("ActiveUsers", 13);
				foreach ($isBacks as $times) {
					if ($times > 0) {
						$count += 1;
					}
				}
				...
				echo $count;
				?> person<?php echo $plural; ?> kommit tillbaka till sin position. Du får vänta.</p>
			...
			<?php
				...
				foreach ($usernames as $name) {
					if ($name != $username) {
						$gottmos = returnFieldForUser("ActiveUsers", 5, $name);
    	...
    	<!-- Iframe checking for updates on the database. Frequency is 25 seconds. -->
		
		<iframe src="iframe.php?frequency=25"></iframe>
